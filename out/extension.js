...
function activate(context) {
    context.subscriptions.push(vscode.window.registerCustomEditorProvider('fancy-file-editor.myTextEditor', new MyTextEditorProvider(context), { webviewOptions: { retainContextWhenHidden: true } }));
}
exports.activate = activate;
class MyTextEditorProvider {
    constructor(context) {
        this.context = context;
        this.tsvModels = new Map();
    }
    async resolveCustomTextEditor(document, webviewPanel, _token) {
        // Create TSV data model for this document
        const tsvModel = new tsvDataModel_1.TSVDataModel(document.getText());
    ...
    updateDocumentAndRefresh(document, webviewPanel, model, focusCell) {
        // Update the document with new TSV content
        const newTsvText = model.toTSV();
        const edit = new vscode.WorkspaceEdit();
        edit.replace(document.uri, new vscode.Range(0, 0, document.lineCount, 0), newTsvText);
    ...
    refreshWebviewWithFolding(webviewPanel, model, focusCell) {
        const visibleRows = model.getVisibleRows();
        webviewPanel.webview.postMessage({
            type: 'refresh',
            data: {
